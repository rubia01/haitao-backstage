...
export default {
	...
	methods: {
		async getCount() {
			...
			// 获取access_token
			let access_token = await this.http.getToken()
			console.log(access_token)
			// 获取菜谱分类总条数
			this.http.postYun('/tcb/databasecount?access_token='+access_token,{
				env:"pro-0c576",
				query: "db.collection(\"re_recipe\").get()"
		...
		getPage() {
			...
			this.http.postYun('/tcb/databasecount?access_token='+access_token,{
				env:"pro-0c576",
				"query": `db.collection(\"re_recipe\").limit(${this.pagesize}).skip(${(this.page-1)*this.pagesize}).get()`
